...
class GenericOAuthenticator(OAuthenticator):
    ...
    def _get_headers(self):
        headers = {"Accept": "application/json", "User-Agent": "JupyterHub", "Scope": "profile"}

        if self.basic_auth:
            b64key = base64.b64encode(
                bytes("{}:{}".format(self.client_id, self.client_secret), "utf8")
            )
            headers.update({"Authorization": "Bearer {}".format(b64key.decode("utf8"))})
        return headers
    ...
    async def authenticate(self, handler, data=None):
        ...
        self.log.debug(user_data_resp_json)
        self.log.debug("username_key:{}".format(self.username_key))
